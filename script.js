...
const GITHUB_RELEASES_URL =
  "https://api.github.com/repos/dStudios-735453/UniPlan_Releases/releases/latest";

function detectOS() {
  const userAgent = navigator.userAgent.toLowerCase();
  if (/iphone|ipad|ipod|android/.test(userAgent)) return "mobile";
  if (/mac/.test(userAgent) && !/iphone|ipad|ipod|android/.test(userAgent))
    return "mac";
...
function updateDownloadUrls() {
  ...
  downloadData.windows.packageManagers.find((p) => p.id === "exe").url =
    findAsset([
      new RegExp(`_${arch === "arm" ? "arm64" : "x64"}-setup\\.exe$`),
    ]);

  downloadData.mac.packageManagers.find((p) => p.id === "dmg").url = findAsset([
    /_universal\.dmg$/,
    /\.dmg$/,
  ]);

  downloadData.linux.packageManagers.find((p) => p.id === "appimage").url =
    findAsset([
      new RegExp(`_${arch === "arm" ? "aarch64" : "amd64"}\\.AppImage$`),
    ]);
}
...
function updateDownloadContent() {
  ...
  const pm = downloadData[currentOS].packageManagers.find(
    (p) => p.id === currentPM,
  );
  ...
  if (pm.type === "command") {
    ...
    if (isMobileDevice()) {
      ...
      if (container && typeof QRCode !== "undefined") {
        ...
        container.innerHTML =
          '<p class="qr-hint">QR code library not loaded. Please refresh.</p>';
      }
    }
  } else {
    if (!pm.url) {
